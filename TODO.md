...
- [x] Lighthouse mobile pass — a11y 100 across landing/feed/recipe-detail; perf 81-93 (landing 93, feed 81, recipe 87). The remaining gap to 90 is React 19 hydration render-delay on the 24-card feed grid; accepted as-is since real-world wall-clock on the 4G + Moto-G test profile is comfortably under 2 s and the bundle work to clear 90 isn't worth the risk.
...
- [x] **JSON-LD fast path for URL imports** (commit `9d65f50`): when a page embeds a schema.org `Recipe` in `<script type="application/ld+json">`, `buildExtractUserContent` feeds the compact JSON-LD payload to the LLM instead of the full ~150K-char HTML. Roughly halves input tokens, drops output verbosity, and cuts latency from ~10–20 s to ~3–6 s on the major recipe blogs that use Yoast / RankMath / WP Recipe Maker / Tasty Recipes. Falls back to the full HTML when JSON-LD is absent, malformed, or empty.
- [x] **URL extraction latency** (resolved by model switch): the May 14 2026 default-primary swap to `gemini-3.1-flash-lite` cut the worst-case URL import from ~55s on `gemini-2.5-flash` to ~20s. Investigated three further levers — `max_output_tokens` clamp, per-field length budget in the prompt, splitting notes capture into a second pass — and rejected all three: the clamp risks mid-JSON truncation that the repair pipeline can't recover from (hard failures), prompt budgets are unreliable (models comply ~60-80% of the time), and a notes-split adds ~150 lines of code plus async-review-payload complexity for marginal wall-clock gain, since the remaining ~20s IS the model writing the chef's notes / headnotes / substitutions we explicitly asked for. Image extraction unaffected (9–12s). If real-world latency complaints surface, the cheapest next step is an observability bump (`[ai.extract.url.slow]` warn line over 15s) so a decision can be made from data.
- [x] **Sanity-check gate for audit hallucinations** (commit `ed25e74`): shipped with both the primary-side check (`primaryReading` field) and a source-side check (corrected qty+unit must appear verbatim in source, with Unicode-fraction normalization). See Phase 12 entry for the full design.
...
- [x] **Paste-text import path + URL-CTA promote** (commit `889087f` + this PR): iOS Safari doesn't honor Web Share Target, so we leaned into pasting as the cross-platform primitive. The URL stage now leads with a full-width "Paste link from clipboard" CTA with the manual input demoted under an "or paste manually" divider; a new "Paste recipe text" tile + `TextStage` handles the "recipe written in my Notes app" case via the same `extractAndVerifyRecipe` pipeline (including the Phase 12 verification gate). `ExtractInput` got a `{kind: "text"}` variant; `defaultModelChainFor` treats text like url (cheap text model, no vision); `ReviewSourcePane` gained an inline collapsible "View source" pane for text imports. Smart link-paste detection on TextStage: if the user pastes a bare URL into the text box, an inline amber banner offers "Switch to URL import" with one tap so we never burn an AI call on a link wrapped in nothing. 25 new tests covering the API request schema (12), the model-chain helper (5), and the text-specific user-content builder (5+3).
